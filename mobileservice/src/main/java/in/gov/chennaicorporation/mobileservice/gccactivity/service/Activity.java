...
public class Activity {
	...
	public List<Map<String, Object>> loadAssetlistbyStreetId(String assetTypeId, String streetId) {
		...
		// *,CONCAT('https://gccservices.chennaicorporation.gov.in/gccofficialapp/files/"+filePath+"',
		// image) AS imageUrl FROM `asset_list` WHERE `category_id`=? AND `streetid`=?";

		/*
		 * String sqlQuery="SELECT al.*, CONCAT('"
		 * +fileBaseUrl+"/gccofficialapp/files/asset_images/2', al.image) AS imageUrl, "
		...
		if ("2".equals(assetTypeId)) {
			 ...
			 * "    al.*, " +
			 * "    CONCAT('" + fileBaseUrl +
			 * "/gccofficialapp/files/', al.image) AS imageUrl, " +
			 * 
						...
						if (questionId != null && !questionId.isEmpty() && !questionId.isBlank()) {
							...
							try {
								...
								if (affectedRows > 0) {
									...
									lastInsertId = (generatedId != null) ? generatedId.intValue() : 0;
									System.out.println(
											"Activity Feedback inserted successfully! Insert Feedback Data ID: "
													+ lastInsertId);
								}
							} catch (DataAccessException e) {
	...
	public String fileUpload(String catId, String fileAction, int lastInsertfeedbackId, MultipartFile file) {
		...
		try {
			...
			String datetimetxt = DateTimeUtil.getCurrentDateTime();
			// File name
			String OrgfileName = catId + "_" + fileAction + "_" + lastInsertfeedbackId + "_" + datetimetxt + "_"
	...
	public List<Map<String, Object>> loadAssetLastFeedbackHistoryByUser(String catId, String assetId, String loginId) {
		...
		if (result.isEmpty()) {
			...
			String sqlQuery3 = "SELECT *,CONCAT('" + fileBaseUrl + "/gccofficialapp/files/" + filePath
					+ "', filename) AS imageUrl "
	...
	public List<Map<String, Object>> loadAssetLastFeedbackByUser(String catId, String assetId, String loginId) {
		...
		String filePath = serviceFolderName + catId + "/";
		String sqlQuery3 = "SELECT *,CONCAT('" + fileBaseUrl + "/gccofficialapp/files/" + filePath
				+ "', filename) AS imageUrl FROM `activity_feedback_file` WHERE feedback_id=?";
		List<Map<String, Object>> result3 = jdbcActivityTemplate.queryForList(sqlQuery3, feedback_id);
	 ...
	 * String sqlQuery =
	 * "SELECT SUM(`totalcount`) as Total FROM `asset_count_master` WHERE catid=? and zone=?"
	 * ;
	 * 
	 * List<Map<String, Object>> taskResult =
	 * jdbcActivityTemplate.queryForList(sqlQuery, catid, zone);
	 * 
	 ...
	 * String sqlQuery =
	 * "SELECT SUM(`totalcount`) as Total FROM `asset_count_master` WHERE catid=? AND zone=? AND ward=?"
	 * ;
	 * 
	 * List<Map<String, Object>> taskResult =
	 * jdbcActivityTemplate.queryForList(sqlQuery, catid, zone, ward);
	 * 
	 * if (!taskResult.isEmpty() && taskResult.get(0).get("Total") != null) {
	 * return ((Number) taskResult.get(0).get("Total")).intValue();
	 * } else {
	...
	public Map<String, Object> filterReports(
		...
		StringBuilder sqlQuery = new StringBuilder("SELECT "
				...
				// "
				// + " COALESCE(SUM(CASE WHEN aff.fileaction = 'before' THEN 1 ELSE 0 END), 0)
				// AS total_started, "
				// + " COALESCE(SUM(CASE WHEN aff.fileaction = 'after' THEN 1 ELSE 0 END), 0) AS
				// total_completed, "
				+ ExtraSQL
		 ...
		 * {
		 * sqlQuery.
		 * append(" AND ((DATE(al.cdate) > '2024-08-01' AND DATE(al.cdate) <= '2025-06-12' AND al.zone IN (1,2,3,4,5,6,7)) OR"
		 * +
		...
		// Add HAVING clause to filter out rows where total_started and total_completed
		// are both zero
		 ...
		 * Map<String, Object> result2 = new HashMap<>();
		 * result2.put("zoneWise", result);
		 * result2.put("overallTotals", overallResult.isEmpty() ?
		 * Collections.singletonMap("total_started", 0) : overallResult.get(0));
		 * 
		 * 
		 * return result2;
		 */
	...
	public List<Map<String, Object>> filterReportsByDate(
		...
		List<Map<String, Object>> taskResult = jdbcActivityTemplate.queryForList(sqlQueryForTasks, fromDate, toDate,
				assetType, streetid);

		// Step 2: For each task, load the detailed feedback similar to
		// loadAssetFeedback method
		for (Map<String, Object> task : taskResult) {
			...
			String sqlQueryForFeedback = "SELECT * FROM `activity_feedback` WHERE (`isactive`=1 AND `isdelete`=0) "
					+ "AND (`asset_id`=" + assetId + " AND id IN (" + feedbackIds
			...
			if (!feedbackResult.isEmpty()) {
				...
				String sqlQueryForFeedbackData = "SELECT *,`activity_questions`.english,`activity_questions`.tamil FROM `activity_feedback_data`, `activity_questions` WHERE feedback_id=? "
						+ "AND `activity_questions`.qid = `activity_feedback_data`.`qid`";
				List<Map<String, Object>> feedbackDataResult = jdbcActivityTemplate
						.queryForList(sqlQueryForFeedbackData, feedback_id);

				// Query to get feedback files for the feedback_id
				String serviceFolderName = environment.getProperty("activity_foldername");
				String filePath = serviceFolderName + catId + "/";
				String sqlQueryForFeedbackFiles = "SELECT *,CONCAT('" + fileBaseUrl + "/gccofficialapp/files/"
						+ filePath + "', filename) "
						+ "AS imageUrl FROM `activity_feedback_file` WHERE feedback_id IN (" + feedbackIds + ")";
				List<Map<String, Object>> feedbackFileResult = jdbcActivityTemplate
						.queryForList(sqlQueryForFeedbackFiles);
	 ...
	 * 
	 * // Step 1: Generate all dates within the specified month and year
	 * String sqlQueryForTasks =
	 * "SELECT `asset_list`.*, `activity_feedback`.`id` feedbackid "
	 ...
	 * // Execute the query to get the tasks
	 * List<Map<String, Object>> taskResult =
	 * jdbcActivityTemplate.queryForList(sqlQueryForTasks, fromDate, toDate,
	 * assetType, streetid);
	 * 
	 * // Step 2: For each task, load the detailed feedback similar to
	 * loadAssetFeedback method
	 ...
	 * 
	 * // Query to get the last feedback by user for the given asset
	 * String sqlQueryForFeedback =
	 * "SELECT * FROM `activity_feedback` WHERE (`isactive`=1 AND `isdelete`=0) "
	 * + "AND (`asset_id`=? AND id=?) ORDER BY `feedback_date` DESC LIMIT 1";
	 * 
	 * // Execute the query and get the feedback result
	 * List<Map<String, Object>> feedbackResult =
	 * jdbcActivityTemplate.queryForList(sqlQueryForFeedback, assetId, feedbackId);
	 * 
	 * if (!feedbackResult.isEmpty()) {
	 * String feedback_id = feedbackResult.get(0).get("id").toString();
	 * 
	 * // Query to get feedback data for the feedback_id
	 * String sqlQueryForFeedbackData =
	 * "SELECT *,`activity_questions`.english,`activity_questions`.tamil FROM `activity_feedback_data`, `activity_questions` WHERE feedback_id=? "
	 * + "AND `activity_questions`.qid = `activity_feedback_data`.`qid`";
	 * List<Map<String, Object>> feedbackDataResult =
	 * jdbcActivityTemplate.queryForList(sqlQueryForFeedbackData, feedback_id);
	 * 
	 * // Query to get feedback files for the feedback_id
	 * String serviceFolderName = environment.getProperty("activity_foldername");
	 * String filePath = serviceFolderName + assetId + "/";
	 * String sqlQueryForFeedbackFiles =
	 * "SELECT *,CONCAT('https://gccservices.chennaicorporation.gov.in/gccofficialapp/files/"
	 * + filePath +
	 * "', filename) AS imageUrl FROM `activity_feedback_file` WHERE feedback_id=?";
	 * List<Map<String, Object>> feedbackFileResult =
	 * jdbcActivityTemplate.queryForList(sqlQueryForFeedbackFiles, feedback_id);
	 * 
	 * // Add feedback data and files to the task result
	 * task.put("feedback_value", feedbackDataResult);
	 * task.put("feedback_file", feedbackFileResult);
	...
	public String getApiResponse() {
		// String
		// url="https://media.smsgupshup.com/GatewayAPI/rest?userid=2000233507&password=h2YjFNcJ&send_to=9176617754&v=1.1&format=json&msg_type=TEXT&method=SENDMESSAGE&msg=Welcome+to+GCC%21+%0A%0AYour+OTP+for+GCC+Community+Centre+is+36837+.";
		...
		// String urlString =
		// "https://tmegov.onex-aura.com/api/sms?key=pfTEYN6H&to=9176617754&from=GCCCRP&body=Welcome
		// to GCC! Your OTP for GCC Community Centre booking is
		// 00000&entityid=1401572690000011081&templateid=1407172346444550398";

		// String urlString =
		// "https://tmegov.onex-aura.com/api/sms?key=pfTEYN6H&to=9176617754&from=GCCCRP&body=Welcome
		// to GCC, Your User Registration for booking Community Centre is
		// successful.&entityid=1401572690000011081&templateid=1407172346621223019";
	 ...
	 * 
	 * StringBuilder sqlQuery = new StringBuilder("SELECT "
	 * + "		CASE WHEN GROUPING("+groupQuery+") = 1 THEN 'TOTAL' ELSE "
	 * +groupQueryClass+" END AS CLASS,"
	 * + "    	CASE WHEN GROUPING("+groupQuery+") = 1 THEN 'TOTAL' ELSE "
	 * +groupQuery+" END AS VARIABLE,"
	 ...
	 * + "                    WHERE ab.asset_id = al.id"
	 * //+ "                      AND ab.feedback_date BETWEEN '"+fromDate+"' AND '"
	 * +toDate+"'"
	 * + "               AND ab.feedback_date >= '" + fromDate + "'"
	 * + "               AND ab.feedback_date < DATE_ADD('" + toDate +
	 * "', INTERVAL 1 DAY)"
	 * + "                )"
	 * + "                THEN 1 ELSE 0"
	 * + "            END"
	 ...
	 * 
	 * List<Map<String, Object>> result =
	 * jdbcActivityTemplate.queryForList(sqlQuery.toString());
	 * 
	 * // --- Final result ---
	...
	public Map<String, Object> scpfilterReports(
		...
		StringBuilder sqlQuery = new StringBuilder("SELECT "
				+ "		CASE WHEN GROUPING(" + groupQuery + ") = 1 THEN 'TOTAL' ELSE " + groupQueryClass
				+ " END AS CLASS,"
				+ "    	CASE WHEN GROUPING(" + groupQuery + ") = 1 THEN 'TOTAL' ELSE " + groupQuery
				+ " END AS VARIABLE,"
				// + " COALESCE("+groupQueryClass+", 'TOTAL') AS CLASS,"
				// + " COALESCE("+groupQuery+", 'TOTAL') AS VARIABLE,"
				+ "        COUNT(al.id) AS Total,"
	...
	public Map<String, Object> scpReportAssetList(
		 ...
		 * +
		 * "    GROUP_CONCAT(DISTINCT CASE WHEN aff.fileaction = 'before' THEN CONCAT('"
		 * + fileBaseUrl + "/gccofficialapp/files/', '" + filePath +
		 * "', aff.filename) END SEPARATOR ',') AS before_filepath, " +
		 * "    GROUP_CONCAT(DISTINCT CASE WHEN aff.fileaction = 'after' THEN CONCAT('"
		 * + fileBaseUrl + "/gccofficialapp/files/', '" + filePath +
		 * "', aff.filename) END SEPARATOR ',') AS after_filepath " +
		 * "FROM asset_list al " +
		...
		StringBuilder sqlQuery = new StringBuilder(
				"SELECT " +
						...
						"    (SELECT CONCAT('" + fileBaseUrl + "/gccofficialapp/files/', '" + filePath
						+ "', aff2.filename) " +
