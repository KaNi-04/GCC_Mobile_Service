...
public class illegalDebrisVehicleService {
	...
	public List<Map<String, Object>> getVehicleTypeList() {
		String sql = "SELECT * FROM `vehicle_type` WHERE isactive=1 ORDER BY `vtypeid`";
	...
	public List<Map<String, Object>> saveCatchVehicleData(
		...
		String insertSqltxt = "INSERT INTO `vehicle_catch_location_mapping` "
				...
				+ "	`vehicle_no`, `vtypeid`,  `place_name`,`tonage`, `typeid`,  `remarks`, `file`,`status`) "
				+ "VALUES "
				+ "(?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		int affectedRows = jdbcTemplate.update(connection -> {
			...
			ps.setInt(i++, vehicle_type);
			ps.setString(i++, place_name);
			ps.setString(i++, tonage);
			ps.setInt(i++, waste_type);
		...
		String vtypeid = String.valueOf(vehicle_type);
		String typeid = String.valueOf(waste_type);

		Map<String, Object> fineResponse = getFineAmountDetails(vtypeid, typeid, tonage);

		String fineAmount = fineResponse.get("amount").toString();

		if (affectedRows > 0) {
			int lastInsertId = keyHolder.getKey().intValue();
			response.put("illegal_debris_vehicle", action + " insertId: " + lastInsertId);
			response.put("fine_amount", fineAmount);
	...
	public List<Map<String, Object>> step3(
		...
		String selectQuery = "SELECT vtypeid, typeid, tonage FROM illegal_debris_vehicle.vehicle_catch_location_mapping where vclid = ? LIMIT 1";
		Map<String, Object> vehicleData = jdbcTemplate.queryForMap(selectQuery, vclid);

		String vtypeid = vehicleData.get("vtypeid").toString();
		String typeid = vehicleData.get("typeid").toString();
		String tonage = (String) vehicleData.get("tonage");
		...
		try {
			...
			if (affectedRows > 0) {
				...
				System.out.println(
						"A new vehicle Penalty (Chellan) was generated successfully! Insert ID: " + generatedId);
	...
	public List<Map<String, Object>> getChallanPOSData(
		...
		if (vehicleNo != null && !vehicleNo.isEmpty() && !vehicleNo.isBlank()) {
			sqlQuery = sqlQuery + " AND vclm.vehicle_no='" + vehicleNo + "'";
		}
		...
		if ("Transaction success".equalsIgnoreCase(transactionResponseStatus)) {
			...
			try {
				...
				if (affectedRows > 0) {
					...
					// Fetch inserted data for response, joining `penalty_challan` and
					// `bank_transactions`
	...
	public List<Map<String, Object>> storeFailedBankTransaction(Map<String, Object> transactionData) {
		...
		if ("Transaction cancelled".equalsIgnoreCase(transactionResponseStatus)) {
			...
			// Convert transaction amount properly
			// Double transactionAmount = Double.valueOf((String)
			// txnResponseData.get("transactionAmount"));
			...
			if (transactionAmountStr != null && !transactionAmountStr.trim().isEmpty()) {
				try {
					...
					// Handle the case where the string cannot be parsed to a Double // Log the
					// error or throw a custom exception
	...
	public List<Map<String, Object>> saveClose(
		...
		try {
			...
			if (affectedRows > 0) {
				...
				// System.out.println("A new complaint reply added successfully! Insert ID: " +
				// generatedId);
			} else {
				response.put("status", "error");
	...
	public List<Map<String, Object>> getCloseSummary(String ward, String fromDate, String toDate) {
		...
		for (Map<String, Object> row : list) {
			...
			if (wt != null && wt instanceof String) {
				try {
					List<Map<String, Object>> wasteTypes = mapper.readValue(wt.toString(),
	...
	public Map<String, Object> getVehicleData(String zone, String ward, String vehicle_no) {
		...
		String sql = """
				    ...
				    wt.name AS comp_type,
				    CONCAT('https://gccservices.in/gccofficialapp/files', vclm.file) AS comp_image,
				                CONCAT('https://gccservices.in/gccofficialapp/files', ydd.image_path) as yard_image
				FROM vehicle_catch_location_mapping vclm
				            left join yard_dump_details ydd on vclm.vclid =  ydd.vclid
				JOIN erp_pgr.EG_USER eu ON vclm.cby = eu.id
				LEFT JOIN waste_type wt ON vclm.typeid = wt.typeid
				WHERE vclm.isactive = 1 AND vclm.status !='Close' AND ydd.zone = ? and ydd.ward = ? and vclm.vehicle_no = ? """;
		Map<String, Object> vehicleList = jdbcTemplate.queryForMap(sql, zone, ward, vehicle_no);

		int vclId = vehicleList.get("vclid") != null ? Integer.parseInt(vehicleList.get("vclid").toString()) : 0;

		String sqlWasteDetails = "select vtypeid, typeid, tonage from vehicle_catch_location_mapping where vclid = ? limit 1";
		Map<String, Object> wasteDetails = jdbcTemplate.queryForMap(sqlWasteDetails, vclId);

		String vtypeid = wasteDetails.get("vtypeid") != null ? wasteDetails.get("vtypeid").toString() : "";
		String typeid = wasteDetails.get("typeid") != null ? wasteDetails.get("typeid").toString() : "";
		String tonage = wasteDetails.get("tonage") != null ? wasteDetails.get("tonage").toString() : "";

		String fineAmount = getFineAmountDetails(vtypeid, typeid, tonage).get("amount").toString();

		vehicleList.put("fine_amount", fineAmount);

		return vehicleList;

	}

	public Map<String, Object> getFineAmountDetails(String vehicletype, String wastetype, String tonage) {
		Map<String, Object> response = new HashMap<>();
		try {
			int vehicleType = Integer.parseInt(vehicletype);
			int wasteType = Integer.parseInt(wastetype);
			double tonageVal = Double.parseDouble(tonage);

			// Rule: If waste type is 4, fine is 5000 regardless of vehicle or tonnage
			if (wasteType == 4) {
				response.put("status", "success");
				response.put("amount", 5000);
				response.put("message", "Fine amount calculated");
				return response;
			}

			// Ceiling the tonnage
			double ceilTonnage = Math.ceil(tonageVal);
			System.out.println("ceilTonnage.." + ceilTonnage);
			String tonnageCategory;

			if (tonageVal <= 1.0) {
				tonnageCategory = "Below 1 ton";
			} else {
				tonnageCategory = "1 ton  & Above";
			}

			System.out.println("tonnageCategory.." + tonnageCategory);
			String sql = "SELECT * FROM amount_config WHERE vtid = ? AND wtid = ? AND tonage = ? LIMIT 1";
			List<Map<String, Object>> configList = jdbcTemplate.queryForList(sql, vehicleType, wasteType,
					tonnageCategory);

			if (!configList.isEmpty()) {
				Map<String, Object> config = configList.get(0);
				double amount = Double.parseDouble(config.get("amount").toString());
				double finalFine = 0;
				...
				if (tonageVal <= 1.0) {
					...
					// Rate per ton
					finalFine = ceilTonnage * amount;
				}

				response.put("status", "success");
				response.put("amount", finalFine);
				response.put("message", "Fine amount calculated");
			} else {
				response.put("status", "error");
				response.put("message", "Configuration not found for vtid:" + vehicleType + ", wtid:" + wasteType
						+ ", cat:" + tonnageCategory);
			}
			...
			e.printStackTrace();
			response.put("status", "error");
			response.put("message", "Error calculating fine: " + e.getMessage());
		}
		return response;
	}

	public List<Map<String, Object>> submitDumpDetails(String vclid, String zone, String ward, String latitude,
			String longitude, MultipartFile dumpImage, String remarks, String login) {

		List<Map<String, Object>> result = new ArrayList<>();
		Map<String, Object> response = new HashMap<>();

		String filetxt = login;

		// For Image
		String filetype = "image";
		String action = "yard_dump";

		// for Image 1
		String file1 = fileUpload(action, filetxt, dumpImage, filetype);
		...
		String insertSqltxt = "insert into yard_dump_details (vclid, zone, ward, latitude, longitude, image_path, remarks, cby) values(?,?,?,?,?,?,?,?)";
